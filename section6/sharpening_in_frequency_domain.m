...
u = 0:(M-1); % here we define the u axis in the plane
idx = find(u>M/2); % notice that the plane is devided, half postive and half negative 
u(idx) = u(idx)-M;
v = 0:(N-1); % here we define the x axis in the plane
idy = find(v>N/2);
v(idy) = v(idy)-N;
...
subplot(2, 1, 1), imshow(input_image), title(''),
